...
RSpec.describe AnswerMatcher do
  describe '.match?' do
    context 'when answers are exactly the same' do
      it 'returns true for identical answers' do
    ...
    context 'when one answer is a substring of the other' do
      it 'returns true if correct answer includes user answer (longer than 3 chars)' do
        expect(AnswerMatcher.match?('World', 'Hello World')).to be true
      end

      it 'returns false if correct answer includes user answer (3 chars or less)' do
        expect(AnswerMatcher.match?('Hi', 'Hello World')).to be false
      end

      it 'returns true if user answer includes correct answer (longer than 3 chars)' do
        expect(AnswerMatcher.match?('Hello Beautiful World', 'World')).to be true
      end
    end

    context 'when answers contain the same words in different order' do
      it 'returns true for answers with the same words in different order' do
        expect(AnswerMatcher.match?('World Hello', 'Hello World')).to be true
      end

      it 'returns true when user answer has extra words' do
        expect(AnswerMatcher.match?('The Hello Beautiful World', 'Hello World')).to be true
      end

      it 'returns true when correct answer has extra words' do
        expect(AnswerMatcher.match?('Hello World', 'The Hello Beautiful World')).to be true
      end
    end

    context 'when answers have matching initials' do
      it 'returns true for answers with matching initials (more than 2 chars)' do
        expect(AnswerMatcher.match?('Happy Wonderful Day', 'Hungry White Dog')).to be true
      end

      it 'returns false for answers with matching initials (2 chars or less)' do
        expect(AnswerMatcher.match?('Happy World', 'Hello World')).to be false
      end
    end

    context 'when answers are similar but not exact' do
